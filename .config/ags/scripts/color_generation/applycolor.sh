...
apply_gtk() {
    if [ ! -f "$TMPL_GRADIENCE" ]; then
        echo "Template file not found for Gradience. Skipping."
        return
    fi
    mkdir -p "$(dirname "$GEN_GRADIENCE")"
    cp "$TMPL_GRADIENCE" "$GEN_GRADIENCE"
    for i in "${!colorlist[@]}"; do
        sed -i "s/{{ ${colorlist[$i]} }}/${colorvalues[$i]}/g" "$GEN_GRADIENCE"
    done
    mkdir -p "$DEST_GRADIENCE_PRESETS"
    gradience-cli apply -p "$GEN_GRADIENCE" --gtk both
